use core::task;
use std::{env, fs::File, io::{Error, ErrorKind, Read, Write}, process::exit};
...
fn main() {
    ...
    let file_data = load_file().unwrap(); 
    ...
    match config.action {
        Action::Add => {
            if let Some(task_text) = config.value {
                ...
                let json_data = serde_json::to_string_pretty(&todo_task)?;

                file_data.file.write_all(json_data.as_bytes());
