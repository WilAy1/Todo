...
impl Config {
    fn build(args: &[String]) -> Result<Config, &str> {
        ...
        let value = if args[2] == "list" || args[2] == "empty" || args[2] == "help" || args[2] == "-h"  {
            None
        } else if args[2] == "done" {
            Some(args[3..].join(" "))
        } else {
    ...
    match config.action {
        ...
        Action::Done => {
            ...
            if let Some(id_str) = config.value {
                let ids_vec: Vec<&str> = id_str.split(' ').collect();
                for an_id in ids_vec { 
                    let id: u32 = match an_id.trim().parse() {
                        Ok(n) => n,
                        Err(_) => {
            ...
            if let Some(id_str) = config.value {
                ...
                if let Value::Array(list) = &mut file_data.content {
                    ...
                    if index.is_some() {
                        ...
                        println!("Todo {} successfully removed", id);
                        return;
                    }

                    println!("x Todo {} does not exist", id);
                }
            }
        ...
        Action::Search => {
            if let Some(query) = config.value {
                if let Value::Array(list) = &mut file_data.content {
                    let mut found = false;
                    println!("Searching '{query}' in tasks...");
                    for task in list {
                        if task["text"].to_string().contains(&query) {
                            ...
                            println!("[{}] {} {}", if task["done"].as_bool().unwrap_or(false) { "✓"} else {" "} , task["id"], task["text"]);
                        }
                    }
                    if !found {
