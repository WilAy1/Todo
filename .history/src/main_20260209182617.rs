...
fn load_file() -> Result<FileData, Error> {
        let task_file_result = OpenOptions::new().read(true).write(true).create(true).open("todo.json");
        ...
        Action::Add => {
            if let Some(task_text) = config.value {
                let next_id = if let Value::Array(list) = &mut file_data.content {
                    let last_task = match list.last().cloned() {
                        Some(task) => task["id"],
                     };
                     let last_id: u32 = last_task["id"].to_string().trim().parse().unwrap() {
                        Ok(num) => num + 1,
                        Err(_) => 1
                     };
                ...
                let todo_task = Todo { id: task_text.len() as u32 + 1 , text: task_text.clone(), done: false };
                ...
                file_data.file.write_all(file_data.content.to_string().as_bytes()).unwrap();
